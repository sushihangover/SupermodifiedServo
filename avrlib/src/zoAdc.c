...
void zoAdcInit(void)
{
	...
	zoAdcSetPrescaler(ZO_ADC_DEFAULT_PRESCALER);	// set default prescaler
	zoAdcSetReference(ZO_ADC_DEFAULT_REFERENCE);	// set default reference
	zoAdcSetTrigger(ZO_ADC_DEFAULT_TRIGGER);		// set free running mode
...
inline bool zoAdcConversionCompleted(void)
{
	return ((bit_is_set(ADCSRA, ADSC)==1)?TRUE:FALSE);
}
